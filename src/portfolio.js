...
export class Portfolio {
  constructor(cfg, logger) {
    ...
    this.realizedPnlSol = 0; // lifetime, for reporting
    this.realizedPnlTodaySol = 0; // today only, drives the daily loss limit
  ...
  rejectEntry(buyAmountSol) {
    ...
    if (this.realizedPnlTodaySol <= -r.dailyLossLimitSol) return 'daily loss limit hit';
  ...
  summary() {
    ...
    const today = `${this.realizedPnlTodaySol >= 0 ? '+' : ''}${this.realizedPnlTodaySol.toFixed(4)}`;
    return (
      `realized P&L ${this.realizedPnlSol >= 0 ? '+' : ''}${this.realizedPnlSol.toFixed(4)} SOL ` +
      `(today ${today}) | ${this.wins}W/${this.losses}L | spent today ${this.spentTodaySol.toFixed(4)} | open: ${open}`
    );
  }
}
